...
// const web3 = new Web3(new Web3.providers.HttpProvider('https://polygon-mumbai.infura.io/v3/d855c3d2acb54bf5a5a967e04e39c730'));
...
const provider1 = new ethers.providers.InfuraProvider('rinkeby');
const provider2 = new ethers.providers.InfuraProvider('maticmum');
...
function provideHandleBlock(tokensData) {
  return async function handleBlock(blockEvent) {
    ...
    findings = await Promise.all(tokensData.map(async (token) => {
      const {
        tokenContract1,
        tokenContract2,
        decimals,
      ...
      const balance2 = await tokenContract2.totalSupply();

      // Keep only 2 decimals because ARIMA doesn't work well with a lot of decimals
      let difference = ethers.utils.formatUnits(balance1.sub(balance2), decimals);
      ...
      // originally was "if (difference > high)""
        if (abs(difference) > 0) {
...
module.exports = {
  ...
  resetLastTimestamp: () => { lastTimestamp = 0; }, // Used in unit tests
};